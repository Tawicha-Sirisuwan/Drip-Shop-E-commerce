...
export default async function CustomersPage(props: {
  ...
  const customers: CustomerData[] = users.map((user: {
    ...
    // Fallback UI data
    const name = user.name || "ไม่ทราบชื่อ";
    const initials = name
      .split(" ")
      .map((n) => n[0])
      .join("")
    ...
    return {
      ...
      initials,
      email: user.email,
      joined: user.createdAt.toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "numeric" }),
      orders: totalOrders,
  ...
  return (
    <main className="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h1 className="font-black text-2xl uppercase">ลูกค้า (Customers)</h1>
        
        <div className="flex items-center gap-3">
          <button className="border border-gray-200 bg-white text-black px-4 py-2.5 rounded-xl font-medium hover:bg-gray-50 transition flex items-center justify-center gap-2 text-sm flex-1 sm:flex-none shadow-sm cursor-pointer">
            <Upload className="w-5 h-5" /> นำเข้าข้อมูล
          </button>
          <button className="bg-black text-white px-5 py-2.5 rounded-xl font-medium hover:bg-gray-800 transition flex items-center justify-center gap-2 text-sm flex-1 sm:flex-none shadow-sm cursor-pointer">
